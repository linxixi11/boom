...
public class PartPanel extends JPanel {
    ...
    public PartPanel() {
        ...
        header.add(UIStyle.sectionLabel("零件库"), BorderLayout.WEST);
        ...
        header.add(UIStyle.buttonRowRight(addBtn, editBtn, copyBtn, delBtn, refreshBtn), BorderLayout.EAST);
        add(header, BorderLayout.NORTH);

        tableModel = new DefaultTableModel(
            new String[]{"ID", "零件编号", "名称", "规格型号", "单位", "材质", "库存数量", "备注"}, 0) {
    ...
    public void refreshData() {
        try {
            ...
            List<Component> parts = componentDao.findByType(Component.TYPE_PART);
    ...
    private void deleteSelected() {
        ...
        if (row < 0) { JOptionPane.showMessageDialog(this, "请先选择一行"); return; }
        Long id = (Long) tableModel.getValueAt(table.convertRowIndexToModel(row), 0);
        if (JOptionPane.showConfirmDialog(this, "确认删除该零件？", "确认", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            try {
    ...
    private void showEditDialog(Component source, boolean copyMode) {
        ...
        String title = source == null ? "新增零件" : (copyMode ? "复制零件" : "编辑零件");
        ...
        String defaultCode = "";
        if (isNew) {
            try { defaultCode = componentDao.nextCode(Component.TYPE_PART); } catch (SQLException ignored) {}
        }
        ...
        addFormRow(form, g, 0, "零件编号", codeField);
